...
- AI 后处理：可选的译文二次处理，支持轻量修正和深度润色两种模式，自动处理 OCR 断行
...
- 你觉得 DeepLX 译文有时生硬或不够自然，想用 AI 做二次润色
...
插件支持在 DeepLX 翻译完成后，调用 AI（通过 OpenRouter）对译文做二次处理。需要填写 OpenRouter API Key，默认关闭。
...
> 两种模式均会自动处理 OCR 断行问题，无需根据输入类型手动切换。
> 短文本（≤20 字符）自动跳过 AI 处理。
> AI 调用失败时自动回退到原始 DeepLX 译文，不会中断翻译流程。
...
| `AI Post-Process` | `关闭` | AI 二次处理模式 |
| `AI API Key` | 空 | OpenRouter API Key |
| `AI Model` | `google/gemini-2.5-flash-lite` | AI 模型，留空使用默认 |
...
需要先在 [openrouter.ai](https://openrouter.ai) 注册并获取 API Key。

- `关闭`：不使用 AI 处理
- `Translation Polish`：轻量修正错译和不自然表达，适合日常翻译
- `Full Refine`：深度润色重组，适合对译文质量要求高的场景

两种模式均会自动处理 OCR 断行，无需手动切换。

默认模型 `google/gemini-2.5-flash-lite`，速度快、成本低。可在 `AI Model` 中填写 [OpenRouter 支持的任意模型](https://openrouter.ai/models)。
...
- `AI Post-Process = Translation Polish` 或 `Full Refine`
- 填写 `AI API Key`
- `Render Mode = Smart Paragraphs`
...
如果你需要”翻译后顺手重写排版”的效果，本质上还是大模型更适合。不过现在你可以通过 `AI Post-Process` 功能来获得类似效果——启用 `Full Refine` 模式即可。
...
AI 后处理集成验证（需要 OpenRouter API Key）：
